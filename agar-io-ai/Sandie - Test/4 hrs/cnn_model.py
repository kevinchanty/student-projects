...
class PolicyGradientAgent(object):
    def __init__(self, ALPHA, GAMMA=0.95, n_actions=6, fc1=256,
                input_shape=(185,95), channels=1, chkpt_dir='tmp/checkpoint',
                gpu={'GPU': 1}):
        self.lr = ALPHA
        self.gamma = GAMMA
    ...
    def build_net(self):
        with tf.variable_scope('parameters'):
            ...
            self.G = tf.placeholder(tf.int32, shape=[None], name='G')
        ...
        with tf.variable_scope('conv_layer_1'):
            ...
            batch1 = tf.layers.batch_normalization(input=conv1, epsilon= 1e-5,
                                                    name='batch1')
            conv1_activated = tf.nn.relu(batch1)

        with tf.variable_scope('conv_layer_2'):
            conv2 = tf.layers.conv2d(imputs=conv1_activated, filters=64,
                                    kernal_size=(4,4), strides=2, name='conv2',
                    kernal_initializer=tf.contrib.layers.xavier_initializer())
            batch1 = tf.layers.batch_normalization(input=conv2, epsilon= 1e-5,
                                                    name='batch2')
        ...
        with tf.variable_scope('conv_layer_3'):
            conv3 = tf.layers.conv2d(inputs=conv2_activated, filters=128,
                                    kernal_size=(3,3), strides=1, name='conv3',
                    kernal_initializer=tf.contrib.layers.xavier_initializer())
            batch3 = tf.layers.batch_normalization(input=conv3, epsilon= 1e-5,
                                                    name='batch3')
        ...
        with tf.variable_scope('loss'):
            np.negative_log_prob = tf.nn.sparse_softmax_cross_entropy_with_logits(
                                    logits=dense2, labels=self.label)

        with tf.variable_scope('train'):
            self.train_op = tf.train.RMSPropOptimizer(self.lr, decay=0.99,
                            momenttum=0.0, epsilon=1e-6).minimize(self.loss)
